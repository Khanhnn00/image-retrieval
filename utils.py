...
class ImageReader(Dataset):
    ...
    def __init__(self, data_path, data_name, data_type, crop_type):
        ...
        data_dict = torch.load('{}/{}/{}/{}_data_dicts.pth'.format(data_path, data_name, data_type, crop_type))
        self.class_to_idx = dict(zip(sorted(data_dict[data_type]), range(len(data_dict[data_type]))))
        ...
        for label, image_list in data_dict[data_type].items():
